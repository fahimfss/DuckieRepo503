...
class ImageSubscriber(DTROS):
    def __init__(self, node_name):
        ...
        # Subscribe to my_published_image topic
        self.sub = rospy.Subscriber(f'/{os.environ['VEHICLE_NAME']}/my_published_image/compressed', CompressedImage, self.callback,  queue_size = 1)

        self.bridge = CvBridge()

    def callback(self, ros_data):
        image = self.bridge.compressed_imgmsg_to_cv2(ros_data)
        rospy.loginfo(f'Received image size: {image.shape}')
